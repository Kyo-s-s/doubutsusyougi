...
public class Board {
            ...
            for (int w = 0; w < BOARD_CELL_WIDTH; w++) {
                if (isPlayer(board[h][w])) {
                    for (Pos pos : getMoves(board[h][w])) {
                        int nextH = h + pos.getFirst();
                        int nextW = w + pos.getSecond();
                        if (!new Pos(nextH, nextW).checkInBoard()) continue;
                        if (board[nextH][nextW] == PieceEnum.LION_ENEMY) {
                            ret -= 1000;
                        }
                    }
                    ...
                    for (Pos pos : getMoves(board[h][w])) {
                        ...
                        int nextW = w + pos.getSecond();
                        if (!new Pos(nextH, nextW).checkInBoard()) continue;
                        if (board[nextH][nextW] == PieceEnum.LION_PLAYER) {
                            ret += 500;
                        }
                    }
                }
    ...
    public ArrayList<Board> transition() {
        ArrayList<Board> ret = new ArrayList<>();
        for (int h = 0; h < BOARD_CELL_HEIGHT; h++) {
            for (int w = 0; w < BOARD_CELL_WIDTH; w++) {
                if (!isPlayer(board[h][w])) continue;
                for (Pos pos : getMoves(board[h][w])) {
                    int nextH = h + pos.getFirst();
                    int nextW = w + pos.getSecond();
                    if (!new Pos(nextH, nextW).checkInBoard()) continue;
                    if (canMovePiece(h, w, nextH, nextW)) {
                        Board next = clone();
                        next.movePiece(h, w, nextH, nextW);
                        ret.add(next);
                    }
                }
            }
        ...
        for (Pair<PieceEnum, Integer> pair : playerHand) {
            for (int h = 0; h < BOARD_CELL_HEIGHT; h++) {
                for (int w = 0; w < BOARD_CELL_WIDTH; w++) {
                    if (putCheck(pair.getFirst(), h, w)) {
                        Board next = clone();
                        next.putPiece(pair.getFirst(), h, w);
                        ret.add(next);
                    }
                }
            }
